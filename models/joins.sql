with
    prod as (
        ...
        left join
            {{ source("sources", "suppliers") }} as sp
            on (pd.supplier_id = sp.supplier_id)
        left join
            {{ source("sources", "categories") }} as ct
    ...
    ordrs as (
        ...
        left join 
            {{ ref("employees") }} as em 
            on (em.employee_id = ord.employee_id)
        left join 
             {{ source("sources", "shippers") }} as sp
             on (sp.shipper_id = ord.ship_via)
    ),
    finaljoin as (
        select
            od.*,
